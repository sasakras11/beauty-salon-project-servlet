...
public class ReservationDaoImpl extends AbstractCrudDaoImpl<Reservation> implements ReservationDao {
  ...
  private static final String SAVE =
      "insert into reservations(end_hour,procedure_id,start_hour,beauty_master_user_id,client_user_id) values(?,?,?,?,?)";
  private static final String UPDATE =
      "update reservations set end_hour = ?,procedure_id= ?,start_hour = ?,beauty_master_user_id = ?,client_user_id = ? where reservation_id = ?";
  private static final String FIND_MASTER_RESERVATIONS =
      "select *from reservations where beauty_master_user_id = ?";
  private static final String FIND_BY_ID = "select *from reservations where reservation_id = ?";

  @Override
  protected void setStatementParams(PreparedStatement statement, Reservation entity)
      throws SQLException {
  ...
  @Override
  protected void setStatementParamsWithId(PreparedStatement statement, Reservation entity)
      throws SQLException {
