...
public class UserServiceImpl implements UserService {
  ...
  public UserServiceImpl(
      UserDao userDao, CredentialsValidator credentialsValidator, PasswordUtil passwordUtil) {
  ...
  public Optional<User> login(String username, String password) {
    ...
    boolean isPresent =
        byUsername.map(User::getPassword).filter(pass -> pass.equals(hashedPassword)).isPresent();
    ...
    if (!(userDao.findByUserName(username).isPresent())) {
      User user =
          User.UserBuilder.anUser()
              ...
              .withPassword(passwordUtil.getHashedPassword(password))
